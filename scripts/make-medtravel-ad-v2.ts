 ...
 *   6. Upload final to R2: demo/medical-tourism-ad/v2/medtravel-final.mp4
 *
 * Usage:
 *   npx tsx scripts/make-medtravel-ad-v2.ts
 */
...
async function generateReferenceImage(): Promise<string> {
  ...
  while (Date.now() - startedAt < TIMEOUT_MS) {
    ...
    const status = (polled as { status?: string }).status;
    if (status === "completed" || status === "success") {
      const outputs = (polled as { outputs?: string[] }).outputs;
      imageUrl = outputs?.[0];
      if (imageUrl) break;
    }
    if (status === "failed") {
...
async function main() {
  ...
  // Step 3: Download + R2
  const tmp = mkdtempSync(join(tmpdir(), "medtravel-v2-"));

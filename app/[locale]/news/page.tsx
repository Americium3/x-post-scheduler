...
export const metadata: Metadata = {
  title: "传媒行业日报 | Media Industry Daily | xPilot",
  description:
    "每日汇总主流媒体与媒体科技行业动态，中英双语，指导产品与营销决策。Bilingual daily media-tech brief guiding product and marketing decisions.",
};
...
// ── product direction signals derived from highlights ─────────────────────────
...
export default async function MediaNewsPage({
  ...
  const [latest, latestWeekly, dailyArchiveRaw] =
    await Promise.all([
      fetchDailyMediaTechNews(),
  ...
  const todayIndustryHighlights = todayHighlightsRaw.filter((h) => !h.startsWith(contextPrefix));
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow overflow-hidden">
            ...
            {latest?.coverImageUrl ? (
              ...
              <img
                ...
                alt={isZh ? "今日传媒行业日报配图" : "Today's media industry brief cover"}
            ...
            <div className="p-5 sm:p-6 space-y-5">
              ...
              <div>
                <h3 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">
                  {latest
                    ? isZh ? latest.titleZh : latest.titleEn
                    : isZh ? "今日日报生成中…" : "Today's brief is being prepared…"}
                </h3>
                {latest && (
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                    {latest?.summaryZh ?? "正在从公开信源抓取并汇总最新传媒行业动态。"}
                  </p>
                </div>
                <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                    {latest?.summaryEn ?? "Collecting and summarizing latest media-industry developments from public sources."}
                  </p>
                </div>
              </div>
        ...
        <section id="product">
          <div className="flex items-center gap-2 mb-3">
            <h2 className="text-xs font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">
              {isZh ? "行业信号 → 产品方向" : "Industry Signals → Product Direction"}
            </h2>
          </div>
            ...
            {todayIndustryHighlights.length > 0 && (
              <div className="space-y-2">
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  {isZh ? "今日行业信号（AI 提炼）" : "Today's Industry Signals (AI-distilled)"}
                </p>
                <ol className="space-y-2">
                  {todayIndustryHighlights.map((item, i) => (
            ...
            <div className="space-y-2 border-t border-gray-100 dark:border-gray-700 pt-4">
              ...
              <ol className="space-y-2">
                {(isZh ? PRODUCT_SIGNALS_ZH : PRODUCT_SIGNALS_EN).map(
                  (signal, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-300">
                      <span className="shrink-0 mt-0.5 text-emerald-500 dark:text-emerald-400 font-bold">→</span>
                      {signal}
                    </li>
                  ),
                )}
              ...
              <div className="space-y-2 border-t border-gray-100 dark:border-gray-700 pt-4">
                ...
                <ol className="space-y-2">
                  {weeklyHighlights.slice(0, 2).map((item, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-400 italic">
